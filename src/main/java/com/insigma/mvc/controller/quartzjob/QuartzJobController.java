...
public class QuartzJobController extends MvcHelper {
	...
	@ResponseBody
	@RequiresRoles("admin")
	public  HashMap<String,Object> querylist(HttpServletRequest request,Model model,@RequestBody QrtzTrigger qrtztrigger) throws Exception {
	...
	@ResponseBody
	@RequiresRoles("admin")
	public AjaxReturnMsg saveorupdate( HttpServletRequest request,Model model,@RequestBody @Valid QrtzTrigger qrtzTrigger,BindingResult result) throws Exception {
		//检验输入
		if (result.hasErrors()){
	...
	@ResponseBody
	@RequiresRoles("admin")
	public AjaxReturnMsg batchdelete(HttpServletRequest request,Model model,@RequestBody QrtzTrigger qrtzTrigger) throws Exception {
